...
pub trait MetadataStore: Send + Sync {
    ...
    fn list(&self, bucket_id: &Uuid, prefix: Option<&str>, limit: usize, offset: usize) -> StorageResult<Vec<StorageObject>>;
    ...
    fn copy(&self, bucket_id: &Uuid, from_path: &str, to_bucket_id: &Uuid, to_path: &str) -> StorageResult<StorageObject>;
}
...
impl MetadataStore for InMemoryMetadataStore {
    ...
    fn list(&self, bucket_id: &Uuid, prefix: Option<&str>, limit: usize, offset: usize) -> StorageResult<Vec<StorageObject>> {
        ...
        let result: Vec<StorageObject> = objects.iter()
    ...
    fn move_object(&self, bucket_id: &Uuid, from_path: &str, to_path: &str) -> StorageResult<()> {
        ...
        let mut object = objects.remove(&from_key)
    ...
    fn copy(&self, bucket_id: &Uuid, from_path: &str, to_bucket_id: &Uuid, to_path: &str) -> StorageResult<StorageObject> {
        ...
        let source = objects.get(&from_key)
            .ok_or_else(|| StorageError::ObjectNotFound(from_path.to_string()))?;
...
impl<H: crate::rest_api::RestHandler + Send + Sync> MetadataStore for DatabaseMetadataStore<H> {
    fn get(&self, bucket_id: &Uuid, path: &str) -> StorageResult<Option<StorageObject>> {
        ...
        params.filters = vec![
            FilterExpr::new("bucket_id".to_string(), FilterOperator::Eq, json!(bucket_id.to_string())),
            FilterExpr::new("path".to_string(), FilterOperator::Eq, json!(path)),
        ];
        params.limit = 1;
        
        let result = self.handler.list("storage_objects", params, &ctx)
    ...
    fn put(&self, object: &StorageObject) -> StorageResult<()> {
        ...
        if self.get(&object.bucket_id, &object.path)?.is_some() {
            // Update existing
            self.handler.update("storage_objects", &object.id.to_string(), data, &ctx)
                .map_err(|e| StorageError::Internal(e.to_string()))?;
        } else {
            // Insert new
            self.handler.insert("storage_objects", data, &ctx)
                .map_err(|e| StorageError::Internal(e.to_string()))?;
        }
    ...
    fn delete(&self, bucket_id: &Uuid, path: &str) -> StorageResult<()> {
        ...
        let object = self.get(bucket_id, path)?
            .ok_or_else(|| StorageError::ObjectNotFound(path.to_string()))?;
        
        let ctx = RlsContext::service_role();
        self.handler.delete("storage_objects", &object.id.to_string(), &ctx)
            .map_err(|e| StorageError::Internal(e.to_string()))?;
    ...
    fn list(&self, bucket_id: &Uuid, prefix: Option<&str>, limit: usize, offset: usize) -> StorageResult<Vec<StorageObject>> {
        ...
        if let Some(p) = prefix {
            params.filters.push(
                FilterExpr::new("path".to_string(), FilterOperator::Like, json!(format!("{}%", p)))
            );
        }
    ...
    fn move_object(&self, bucket_id: &Uuid, from_path: &str, to_path: &str) -> StorageResult<()> {
        ...
        let mut object = self.get(bucket_id, from_path)?
            .ok_or_else(|| StorageError::ObjectNotFound(from_path.to_string()))?;
    ...
    fn copy(&self, bucket_id: &Uuid, from_path: &str, to_bucket_id: &Uuid, to_path: &str) -> StorageResult<StorageObject> {
        let source = self.get(bucket_id, from_path)?
            .ok_or_else(|| StorageError::ObjectNotFound(from_path.to_string()))?;
...
mod tests {
    ...
    fn test_inmemory_list() {
        ...
        store.put(&create_test_object(bucket_id, "folder/a.txt")).unwrap();
        store.put(&create_test_object(bucket_id, "folder/b.txt")).unwrap();
        store.put(&create_test_object(bucket_id, "other/c.txt")).unwrap();
    ...
    fn test_inmemory_move() {
        ...
        store.put(&object).unwrap();
        store.move_object(&bucket_id, "old/path.txt", "new/path.txt").unwrap();
    ...
    fn test_inmemory_copy() {
        ...
        store.put(&object).unwrap();
        let copied = store.copy(&bucket_id, "source.txt", &bucket_id, "dest.txt").unwrap();
