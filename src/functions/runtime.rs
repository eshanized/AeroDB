...
impl WasmRuntime for StubRuntime {
    fn execute(
        ...
        Ok(ExecutionResult::success(context.invocation_id, result, duration_ms)
...
mod tests {
    ...
    fn test_stub_runtime_failure() {
        ...
        let result = runtime.execute(&function, json!({}), context, &config).unwrap();
        assert!(!result.success);
    ...
    fn test_host_env_get() {
        ...
        assert_eq!(host::env_get("MY_VAR", &context), Some("my_value".to_string()));
