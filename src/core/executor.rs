...
use crate::core::operation::{
    DeleteOp, Operation, QueryOp, ReadOp, UpdateOp, WriteOp,
};
...
impl OperationExecutor for UnifiedExecutor {
    fn execute(
        ...
        Box::pin(async move {
            match op_clone {
                Operation::Read(read) => {
                    execute_read(&storage, &read, &rls_filters)
                }
                Operation::Write(write) => {
                    execute_write(&storage, &write)
                }
                Operation::Update(update) => {
                    execute_update(&storage, &update, &rls_filters)
                }
                Operation::Delete(delete) => {
                    execute_delete(&storage, &delete, &rls_filters)
                }
                Operation::Query(query) => {
                    execute_query(&storage, &query, &rls_filters)
                }
...
pub struct InMemoryStorage {
    data: std::sync::RwLock<std::collections::HashMap<String, std::collections::HashMap<String, Value>>>,
}
