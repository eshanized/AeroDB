...
use crate::auth::rls::{RlsContext, RlsEnforcer, DefaultRlsEnforcer};
...
impl<E: RlsEnforcer + Send + Sync> RestHandler for DatabaseFacade<E> {
    ...
    fn insert(
        ...
        let id = if let Some(id) = data.get("_id").and_then(|v| v.as_str()) {
            ...
            data.as_object_mut().unwrap().insert("_id".to_string(), Value::String(id.clone()));
            id
        };
    ...
    fn update(
        ...
        let existing = coll.documents.get(id)
            .ok_or(RestError::NotFound)?;
        ...
        self.rls.validate_write(collection, &updated, ctx)
            .map_err(RestError::Auth)?;
...
mod tests {
    ...
    fn test_rls_filtering() {
        let rls = DefaultRlsEnforcer::new()
            .with_policy("posts", RlsPolicy::Ownership {
        ...
        db.insert("posts", json!({"title": "User1 Post", "owner_id": user1.to_string()}), &ctx1).unwrap();
        
        // Insert as user2 (must include owner_id for ownership policy)
        let user2 = Uuid::new_v4();
        let ctx2 = RlsContext::authenticated(user2);
        db.insert("posts", json!({"title": "User2 Post", "owner_id": user2.to_string()}), &ctx2).unwrap();
    ...
    fn test_pagination() {
        ...
        for i in 0..10 {
            db.insert("items", json!({"_id": format!("id-{}", i), "idx": i}), &ctx).unwrap();
        }
