...
fn test_r2_recovery_determinism_same_wal_same_result() {
    ...
    for i in 0..pass1_records.len() {
        assert_eq!(
            pass1_records[i].sequence_number,
            pass2_records[i].sequence_number,
            "R2 VIOLATION: Sequence mismatch at index {}", i
        );
        assert_eq!(
            pass2_records[i].sequence_number,
            pass3_records[i].sequence_number,
            "R2 VIOLATION: Sequence mismatch at index {}", i
        );
        assert_eq!(
            pass1_records[i].payload,
            pass2_records[i].payload,
            "R2 VIOLATION: Payload mismatch at index {}", i
        );
    }
}
...
fn test_r2_recovery_preserves_write_order() {
    ...
    let expected_order: Vec<String> = (1..=10)
        .map(|i| format!("doc{}", i))
        .collect();
...
fn test_r2_interleaved_record_types_maintain_order() {
    ...
    // Write interleaved INSERT, UPDATE, DELETE
    {
        ...
        writer.append_delete(WalPayload::tombstone("test", "doc1", "s", "v1")).unwrap();  // seq 4
